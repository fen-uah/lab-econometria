...
[Descargar PDF de contenidos teóricos](pdf_epg/Capitulo_3_Regresion_Multiple.pdf){download="true"}
...
- Predicción:  
  - Fórmula teórica $ \hat{y}_0 = x_0'\hat{\beta} $  
...
$$
\text{Costos} = \beta_0 + \beta_1(\text{Horas_maquina}) + \beta_2(\text{Numero_preparaciones}) + \varepsilon
$$
...
::: {.callout-note}
- **Prueba individual (t test):** evalúa si cada $\beta_j$ es distinto de 0.  
- **Prueba global (F test):** evalúa si el modelo en conjunto explica la variable dependiente.  
- **Signo de $\beta_j$:** indica dirección del efecto.  
...
# Estadístico F calculado a mano
...
### Análisis de residuos modelo sin constante
